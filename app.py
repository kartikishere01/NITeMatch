...
def mark_messages_as_read(chat_id, current_user_id):
    ...
    FIX: Fetch all messages and filter in Python to avoid Firestore index requirements
    """
    try:
...
def get_unread_count(chat_id, current_user_id):
    ...
    try:
        ...
        for msg in messages:
            msg_data = msg.to_dict()
            # Count messages that are: (1) not from current user AND (2) unread AND (3) valid
            if (msg_data.get("sender_id") != current_user_id and 
                msg_data.get("read") == False and 
...
if st.session_state.logged_in and st.session_state.current_user:
    ...
    if now >= UNLOCK_TIME:
        ...
        <div class="glass">
            <div style="text-align:center;">
                <h2 style="margin-bottom:0.5rem;">Welcome back, {current_user.get('alias', 'User')}! 👋</h2>
        ...
        with col2:
            # Count total unread messages - with error handling
            total_unread = 0
            try:
                ...
                # If unread count fails, just show Messages without count
                # print(f"Error getting total unread: {e}")
                pass
            ...
            if matches:
                ...
                for idx, match in enumerate(matches, 1):
                    ...
                    try:
                        ...
                        # If unread count fails, default to 0
                        # print(f"Error getting unread for match {idx}: {e}")
                        unread = 0
                    ...
                    <div class="glass match-glow">
                        ...
                        <div style="background:rgba(255,255,255,0.05);padding:16px;border-radius:12px;margin-bottom:16px;">
                            <div style="font-size:1.5rem;font-weight:700;text-align:center;color:#00ffe1;">
                                {matched_user.get('alias', 'Unknown')}
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    if matched_user.get("match_message"):
                        ...
                        <div style="background: linear-gradient(135deg, rgba(255,79,216,0.15), rgba(0,255,225,0.15)); padding: 18px; border-radius: 12px; margin-bottom: 12px; border-left: 4px solid #ff4fd8;">
                            <div style="font-weight: 600; margin-bottom: 8px; color: #ff4fd8;">💬 Message from {matched_user.get('alias', 'Match')}:</div>
                    ...
                    # Chat button
                    chat_btn_label = f"💬 Chat with {matched_user.get('alias', 'this match')}"
                    if unread > 0:
            ...
            if not matches:
                ...
                if not st.session_state.active_chat:
                    ...
                    for match in matches:
                        ...
                        btn_label = f"💬 {matched_user.get('alias', 'Match')}"
                        if unread > 0:
        ...
        <div class="countdown-box">
            <div class="countdown-title">Welcome back, {current_user.get('alias', 'User')}! 👋</div>
            ...
            if direct_login_btn:
                if not login_alias or not login_email:
                    ...
                    with st.spinner("Logging in..."):
                        ...
                        # Find user by email hash and alias (case-insensitive)
                        users = fetch_users()
                        user = next((u for u in users 
                                   if u.get("email_hash") == login_hash 
                                   and u.get("alias", "").lower() == login_alias.lower()), None)
